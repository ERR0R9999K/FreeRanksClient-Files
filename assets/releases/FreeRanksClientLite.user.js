// ==UserScript==
// @name         FreeRanksClient 4.6.0 Lite / BETA⚠️
...
(function() {
    ...
    const FreeRanksClientLogo = "https://err0r9999k.github.io/FreeRanksClient/assets/images/FreeRanksClientLite.png";
    ...
    const WelcomeMessage = `[FRC] 🚀 FreeRanksClient Lite ${ClientVersion} initialized! Сode cleanup & Size reduction!`;
    ...
    function PatchCanvas() {
        ...
        function buildPatternsFromPlayers() {
            if (!players || players.length === 0) return;

            for (const key in patterns) { delete patterns[key]; }

            players.forEach(player => {
                if (!player.nickname) return;
                let textColor = resolveNicknameColor(player.nickname_color) || null;
                patterns[player.nickname] = { textColor: textColor };
            });
        }
        ...
        HTMLCanvasElement.prototype.getContext = function(type, ...args) {
            ...
            if (type !== '2d' || ctx._hooked) return ctx;
            ctx._hooked = true;

            const originalFillText = ctx.fillText;

            ctx.fillText = function(text, x, y, maxWidth) {
                if (players.length > 0 && Object.keys(patterns).length !== players.length) {
                    buildPatternsFromPlayers();
                }

                const cfg = patterns[text];
                const isNametag = (x >= 0 && x <= 800 && y >= 30 && y <= 300);
...
/* Logos */
.HomeHeaderTop .TitleContainer .Title {
	background: url('https://err0r9999k.github.io/FreeRanksClient-Files/text_logo_mini.png') !important;
...
.InGameHeaderLogo::before {
    ...
    height: 100%;
    width: 110%;
    background-image: url('https://err0r9999k.github.io/FreeRanksClient-Files/text_logo_mini.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center right;
}

.LoadingOverlay .LoadingOverlayRight .LoadingOverlayRightBody {
    position: relative;
    padding-top: 6rem;
}

.LoadingOverlay .LoadingOverlayRight .LoadingOverlayRightBody::before {
    content: '';
    position: absolute;
    top: calc(50% - 15rem);
    left: 50%;
    ...
    background-image: url('https://err0r9999k.github.io/FreeRanksClient-Files/text_logo.png');
    background-size: contain;
    background-repeat: no-repeat;
